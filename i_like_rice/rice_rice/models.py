...
class Rice(models.Model) :
    ...
    current_rices = models.IntegerField(max_length=10, blank=True, null=True, default=0, verbose_name='current_time')
...
class RicePic(models.Model):
    ...
    def save_pic(cls, f, rice_id):
        ...
        if f is not None and Rice.objects.filter(pk=int(rice_id)).exists():
            ...
            save_name = "%s" % int(time())
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            url_path = os.path.join(settings.PHOTO_URL, s_path+save_name)
            cls.objects.create(name=f.name, path=url_path, rice_id=rice_id)
            img = Image.open(f)
